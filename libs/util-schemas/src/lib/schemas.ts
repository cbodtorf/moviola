...
const baseAddMovieSchema = {
  ...
  alternative_titles: array(string().required()).max(50).default(() => []),
  ...
  actors: array(string().required()).max(50).default(() => []),
  actor_facets: array(string()).max(50).default(() => []),
  genre: array(string().required()).max(5).default(() => []),
}
...
export const movieAddFastifySchema = createYupSchema((_yup) => {
  return {
    body: object({
      movie: object({
        ...baseAddMovieSchema
...
export const movieUpdateFastifySchema = createYupSchema((_yup) => {
  return {
    body: object({
      movie: object({
        ...baseUpdateMovieSchema
